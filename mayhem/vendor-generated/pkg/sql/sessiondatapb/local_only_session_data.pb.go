...
type LocalOnlySessionData struct {
	...
	// canary statistics (mode == force_canary), stable statistics
	// (mode == force_stable), or let the system decide based on
	// the canary fraction cluster setting (mode == auto).
	...
	// PgDumpCompatibility controls how CockroachDB presents its metadata to
	// external tools. Valid values are "off" (default), "postgres" (suppress
	// CRDB-specific syntax for pg_dump compatibility), and "cockroachdb"
	// (show CRDB syntax explicitly).
	PgDumpCompatibility string `protobuf:"bytes,206,opt,name=pg_dump_compatibility,json=pgDumpCompatibility,proto3" json:"pg_dump_compatibility,omitempty"`
	...
	OptimizerSpanLimit int32 `protobuf:"varint,207,opt,name=optimizer_span_limit,json=optimizerSpanLimit,proto3" json:"optimizer_span_limit,omitempty"`
	// DistSQLPlanLocalityFilter restricts the set of SQL instances that the
	// DistSQL physical planner will consider when assigning flow components for
	// queries from this session. The value is a CockroachDB locality string
	// (e.g. "region=us-east1,zone=us-east1-a"); only instances whose locality
	// matches every key/value pair in the filter are eligible. The empty string
	// disables the filter and is the default. The filter is consumed only on the
	// gateway during physical planning, so it does not need to be propagated to
	// remote nodes.
	DistSQLPlanLocalityFilter string `protobuf:"bytes,208,opt,name=distsql_plan_locality_filter,json=distsqlPlanLocalityFilter,proto3" json:"distsql_plan_locality_filter,omitempty"`
	// DistSQLPlanLocalityFilterStrict, when true, causes physical planning to
	...
	DistSQLPlanLocalityFilterStrict bool `protobuf:"varint,209,opt,name=distsql_plan_locality_filter_strict,json=distsqlPlanLocalityFilterStrict,proto3" json:"distsql_plan_locality_filter_strict,omitempty"`
	...
	OptimizerInlinePlaceholderEqualities bool `protobuf:"varint,210,opt,name=optimizer_inline_placeholder_equalities,json=optimizerInlinePlaceholderEqualities,proto3" json:"optimizer_inline_placeholder_equalities,omitempty"`
	// ResourceGroupName is the user-visible name of the resource group bound
	// to the current session (e.g. set via SET resource_group = '...'). It is
	// empty when no group is set. The value is consumed on the gateway: it is
	// resolved to ResourceGroupID at SET time and that id is stamped onto the
	// admission header of the session's KV requests, so the name itself does
	// not need to be propagated to remote nodes.
	ResourceGroupName string `protobuf:"bytes,211,opt,name=resource_group_name,json=resourceGroupName,proto3" json:"resource_group_name,omitempty"`
	// ResourceGroupID is the numeric id that ResourceGroupName resolved to, as
	// returned by the resource group name cache. It is zero when no group is
	// set. See ResourceGroupName for how it is used.
	ResourceGroupID uint64 `protobuf:"varint,212,opt,name=resource_group_id,json=resourceGroupId,proto3" json:"resource_group_id,omitempty"`
}
...
var fileDescriptor_21ead158cf36da28 = []byte{
	// 6616 bytes of a gzipped FileDescriptorProto
	,
}
...
func (m *LocalOnlySessionData) MarshalToSizedBuffer(dAtA []byte) (int, error) {
	...
	var l int
	_ = l
	if m.ResourceGroupID != 0 {
		i = encodeVarintLocalOnlySessionData(dAtA, i, uint64(m.ResourceGroupID))
		i--
		dAtA[i] = 0xd
		i--
		dAtA[i] = 0xa0
	...
	if len(m.ResourceGroupName) > 0 {
		...
		i = encodeVarintLocalOnlySessionData(dAtA, i, uint64(len(m.ResourceGroupName)))
		i--
		dAtA[i] = 0xd
		i--
		dAtA[i] = 0x9a
	}
	if m.OptimizerInlinePlaceholderEqualities {
		i--
		if m.OptimizerInlinePlaceholderEqualities {
			dAtA[i] = 1
		} else {
	...
	if m.DistSQLPlanLocalityFilterStrict {
		i--
		if m.DistSQLPlanLocalityFilterStrict {
			dAtA[i] = 1
		} else {
	...
	if len(m.DistSQLPlanLocalityFilter) > 0 {
		i -= len(m.DistSQLPlanLocalityFilter)
		copy(dAtA[i:], m.DistSQLPlanLocalityFilter)
		i = encodeVarintLocalOnlySessionData(dAtA, i, uint64(len(m.DistSQLPlanLocalityFilter)))
		i--
		dAtA[i] = 0xd
		i--
		dAtA[i] = 0x82
	}
	if m.OptimizerSpanLimit != 0 {
		i = encodeVarintLocalOnlySessionData(dAtA, i, uint64(m.OptimizerSpanLimit))
		i--
		dAtA[i] = 0xc
		i--
		dAtA[i] = 0xf8
	}
	if len(m.PgDumpCompatibility) > 0 {
		i -= len(m.PgDumpCompatibility)
		copy(dAtA[i:], m.PgDumpCompatibility)
		i = encodeVarintLocalOnlySessionData(dAtA, i, uint64(len(m.PgDumpCompatibility)))
		i--
		dAtA[i] = 0xc
		i--
		dAtA[i] = 0xf2
...
func (m *LocalOnlySessionData) Size() (n int) {
	...
	l = len(m.PgDumpCompatibility)
	if l > 0 {
		n += 2 + l + sovLocalOnlySessionData(uint64(l))
	}
	if m.OptimizerSpanLimit != 0 {
		n += 2 + sovLocalOnlySessionData(uint64(m.OptimizerSpanLimit))
	}
	l = len(m.DistSQLPlanLocalityFilter)
	if l > 0 {
		n += 2 + l + sovLocalOnlySessionData(uint64(l))
	}
	if m.DistSQLPlanLocalityFilterStrict {
	...
	if m.ResourceGroupID != 0 {
		n += 2 + sovLocalOnlySessionData(uint64(m.ResourceGroupID))
	}
	return n
}
...
func (m *LocalOnlySessionData) Unmarshal(dAtA []byte) error {
	...
	for iNdEx < l {
		...
		case 206:
			if wireType != 2 {
				return fmt.Errorf("proto: wrong wireType = %d for field PgDumpCompatibility", wireType)
			}
			var stringLen uint64
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
			...
			m.PgDumpCompatibility = string(dAtA[iNdEx:postIndex])
			iNdEx = postIndex
		case 207:
			if wireType != 0 {
				return fmt.Errorf("proto: wrong wireType = %d for field OptimizerSpanLimit", wireType)
			}
			m.OptimizerSpanLimit = 0
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
					return ErrIntOverflowLocalOnlySessionData
				}
				...
				b := dAtA[iNdEx]
				iNdEx++
				m.OptimizerSpanLimit |= int32(b&0x7F) << shift
				if b < 0x80 {
					break
				}
			}
		case 208:
			if wireType != 2 {
				return fmt.Errorf("proto: wrong wireType = %d for field DistSQLPlanLocalityFilter", wireType)
			}
			var stringLen uint64
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
			...
			m.DistSQLPlanLocalityFilter = string(dAtA[iNdEx:postIndex])
			iNdEx = postIndex
		case 209:
			if wireType != 0 {
				return fmt.Errorf("proto: wrong wireType = %d for field DistSQLPlanLocalityFilterStrict", wireType)
			}
			var v int
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
			...
			m.DistSQLPlanLocalityFilterStrict = bool(v != 0)
		case 210:
			if wireType != 0 {
				return fmt.Errorf("proto: wrong wireType = %d for field OptimizerInlinePlaceholderEqualities", wireType)
			}
			var v int
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
			...
			m.OptimizerInlinePlaceholderEqualities = bool(v != 0)
		case 211:
			if wireType != 2 {
		...
		case 212:
			...
			for shift := uint(0); ; shift += 7 {
				...
				b := dAtA[iNdEx]
				iNdEx++
				m.ResourceGroupID |= uint64(b&0x7F) << shift
				if b < 0x80 {
					break
				}
			}
