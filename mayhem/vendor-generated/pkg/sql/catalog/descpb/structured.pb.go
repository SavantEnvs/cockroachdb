...
const (
	...
	// Represents a user-defined domain type (a base type with constraints).
	TypeDescriptor_DOMAIN TypeDescriptor_Kind = 5
)
...
func (TypeDescriptor_Kind) EnumDescriptor() ([]byte, []int) {
	return fileDescriptor_12dcc21c3bcc9571, []int{21, 0}
}
...
func (TypeDescriptor_EnumMember_Capability) EnumDescriptor() ([]byte, []int) {
	return fileDescriptor_12dcc21c3bcc9571, []int{21, 0, 0}
}
...
func (TypeDescriptor_EnumMember_Direction) EnumDescriptor() ([]byte, []int) {
	return fileDescriptor_12dcc21c3bcc9571, []int{21, 0, 1}
}

// NotNullState describes how strictly a domain's NOT NULL constraint is
// enforced.
type TypeDescriptor_Domain_NotNullState int32

const (
	// NONE indicates the domain has no NOT NULL constraint.
	TypeDescriptor_Domain_NONE TypeDescriptor_Domain_NotNullState = 0
	// ENFORCING indicates the constraint is fully enforced; existing rows
	// have been validated.
	TypeDescriptor_Domain_ENFORCING TypeDescriptor_Domain_NotNullState = 1
	// VALIDATING indicates the constraint is being added: Writes check it but
	// existing rows have not yet been validated.
	TypeDescriptor_Domain_VALIDATING TypeDescriptor_Domain_NotNullState = 2
	// DROPPING indicates the constraint is being removed: Writes continue to
	// check it until the drop is complete.
	TypeDescriptor_Domain_DROPPING TypeDescriptor_Domain_NotNullState = 3
)

var TypeDescriptor_Domain_NotNullState_name = map[int32]string{
	0: "NONE",
	1: "ENFORCING",
	2: "VALIDATING",
	3: "DROPPING",
...
var TypeDescriptor_Domain_NotNullState_value = map[string]int32{
	"NONE":       0,
	"ENFORCING":  1,
	"VALIDATING": 2,
	"DROPPING":   3,
...
func (x TypeDescriptor_Domain_NotNullState) Enum() *TypeDescriptor_Domain_NotNullState {
	p := new(TypeDescriptor_Domain_NotNullState)
	*p = x
	return p
}

func (x TypeDescriptor_Domain_NotNullState) String() string {
	return proto.EnumName(TypeDescriptor_Domain_NotNullState_name, int32(x))
}

func (x *TypeDescriptor_Domain_NotNullState) UnmarshalJSON(data []byte) error {
	value, err := proto.UnmarshalJSONEnum(TypeDescriptor_Domain_NotNullState_value, data, "TypeDescriptor_Domain_NotNullState")
	if err != nil {
		return err
	}
	*x = TypeDescriptor_Domain_NotNullState(value)
	return nil
}

func (TypeDescriptor_Domain_NotNullState) EnumDescriptor() ([]byte, []int) {
	return fileDescriptor_12dcc21c3bcc9571, []int{21, 3, 0}
}
...
type UniqueWithoutIndexConstraint struct {
	...
	Predicate Expression `protobuf:"bytes,5,opt,name=predicate,casttype=Expression" json:"predicate"`
	// Used within the table descriptor to uniquely identify individual
	// constraints.
...
type ColumnDescriptor struct {
	...
	DefaultExpr *Expression `protobuf:"bytes,5,opt,name=default_expr,json=defaultExpr,casttype=Expression" json:"default_expr,omitempty"`
	...
	// have been serialized in a internal format. Instead, use one of the
	// schemaexpr.FormatExpr* functions.
	OnUpdateExpr *Expression `protobuf:"bytes,18,opt,name=on_update_expr,json=onUpdateExpr,casttype=Expression" json:"on_update_expr,omitempty"`
	...
	// have been serialized in a internal format. Instead, use one of the
	// schemaexpr.FormatExpr* functions.
	ComputeExpr *Expression `protobuf:"bytes,11,opt,name=compute_expr,json=computeExpr,casttype=Expression" json:"compute_expr,omitempty"`
...
type IndexDescriptor struct {
	...
	Type idxtype.T `protobuf:"varint,16,opt,name=type,enum=cockroach.sql.sem.idxtype.T" json:"type"`
	...
	Predicate Expression `protobuf:"bytes,23,opt,name=predicate,casttype=Expression" json:"predicate"`
	...
	// this vector index.
	VecConfig vecpb.Config `protobuf:"bytes,30,opt,name=vec_config,json=vecConfig" json:"vec_config"`
	// HideForPrimaryKeyRecreate, if set this index is not usable for reads until the
	// new primary index becomes public.
	HideForPrimaryKeyRecreate bool `protobuf:"varint,31,opt,name=hide_for_primary_key_recreate,json=hideForPrimaryKeyRecreate" json:"hide_for_primary_key_recreate"`
	// SkipUniqueChecks, if set to true, disables unique constraint checks for
	// this index in the optimizer. This is an unsafe optimization that should
	// only be used when the application guarantees uniqueness externally (e.g.,
	// through application-level logic or regional isolation). When enabled, the
	// optimizer will not plan unique checks for mutations that could violate
	// this index's uniqueness constraint.
	SkipUniqueChecks bool `protobuf:"varint,32,opt,name=skip_unique_checks,json=skipUniqueChecks" json:"skip_unique_checks"`
}
...
type TriggerDescriptor struct {
	// Used within the table descriptor to uniquely identify individual
	// constraints.
	ID TriggerID `protobuf:"varint,1,opt,name=id,casttype=TriggerID" json:"id"`
	...
	ActionTime semenumpb.TriggerActionTime `protobuf:"varint,3,opt,name=action_time,json=actionTime,enum=cockroach.sql.sem.semenumpb.TriggerActionTime" json:"action_time"`
	...
	Events []*TriggerDescriptor_Event `protobuf:"bytes,4,rep,name=events" json:"events,omitempty"`
	// Optional alias for the mutated set of rows before the mutation. Empty if
	// an alias was not specified.
	NewTransitionAlias string `protobuf:"bytes,5,opt,name=new_transition_alias,json=newTransitionAlias" json:"new_transition_alias"`
	// Optional alias for the mutated set of rows after the mutation. Empty if
	// an alias was not specified.
	OldTransitionAlias string `protobuf:"bytes,6,opt,name=old_transition_alias,json=oldTransitionAlias" json:"old_transition_alias"`
	...
	ForEachRow bool `protobuf:"varint,7,opt,name=for_each_row,json=forEachRow" json:"for_each_row"`
	// An optional boolean condition that must be satisfied for the trigger to
	// execute. Empty if a WHEN clause was not specified.
	WhenExpr Expression `protobuf:"bytes,8,opt,name=when_expr,json=whenExpr,casttype=Expression" json:"when_expr"`
	// The ID of the function that is executed when the trigger is fired.
	FuncID ID `protobuf:"varint,9,opt,name=func_id,json=funcId,casttype=ID" json:"func_id"`
	// String constant arguments for the trigger function.
	FuncArgs []string `protobuf:"bytes,10,rep,name=func_args,json=funcArgs" json:"func_args,omitempty"`
	...
	FuncBody RoutineBody `protobuf:"bytes,11,opt,name=func_body,json=funcBody,casttype=RoutineBody" json:"func_body"`
	// If true, indicates that the trigger is active.
	Enabled bool `protobuf:"varint,12,opt,name=enabled" json:"enabled"`
	...
	// trigger depends on.
	DependsOnRoutines []ID `protobuf:"varint,15,rep,name=depends_on_routines,json=dependsOnRoutines,casttype=ID" json:"depends_on_routines,omitempty"`
}

func (m *TriggerDescriptor) Reset()         { *m = TriggerDescriptor{} }
func (m *TriggerDescriptor) String() string { return proto.CompactTextString(m) }
...
func (m *TriggerDescriptor) XXX_Unmarshal(b []byte) error {
	return m.Unmarshal(b)
}
func (m *TriggerDescriptor) XXX_Marshal(b []byte, deterministic bool) ([]byte, error) {
	b = b[:cap(b)]
	n, err := m.MarshalToSizedBuffer(b)
	if err != nil {
		return nil, err
...
func (m *TriggerDescriptor) XXX_Merge(src proto.Message) {
	xxx_messageInfo_TriggerDescriptor.Merge(m, src)
}
func (m *TriggerDescriptor) XXX_Size() int {
	return m.Size()
}
func (m *TriggerDescriptor) XXX_DiscardUnknown() {
	xxx_messageInfo_TriggerDescriptor.DiscardUnknown(m)
}

var xxx_messageInfo_TriggerDescriptor proto.InternalMessageInfo

type TriggerDescriptor_Event struct {
	Type        semenumpb.TriggerEventType `protobuf:"varint,1,opt,name=type,enum=cockroach.sql.sem.semenumpb.TriggerEventType" json:"type"`
	ColumnNames []string                   `protobuf:"bytes,2,rep,name=column_names,json=columnNames" json:"column_names,omitempty"`
}

func (m *TriggerDescriptor_Event) Reset()         { *m = TriggerDescriptor_Event{} }
func (m *TriggerDescriptor_Event) String() string { return proto.CompactTextString(m) }
func (*TriggerDescriptor_Event) ProtoMessage()    {}
func (*TriggerDescriptor_Event) Descriptor() ([]byte, []int) {
	return fileDescriptor_12dcc21c3bcc9571, []int{7, 0}
}
func (m *TriggerDescriptor_Event) XXX_Unmarshal(b []byte) error {
	return m.Unmarshal(b)
}
func (m *TriggerDescriptor_Event) XXX_Marshal(b []byte, deterministic bool) ([]byte, error) {
	b = b[:cap(b)]
	n, err := m.MarshalToSizedBuffer(b)
	if err != nil {
		return nil, err
...
func (m *TriggerDescriptor_Event) XXX_Merge(src proto.Message) {
	xxx_messageInfo_TriggerDescriptor_Event.Merge(m, src)
}
func (m *TriggerDescriptor_Event) XXX_Size() int {
	return m.Size()
}
func (m *TriggerDescriptor_Event) XXX_DiscardUnknown() {
	xxx_messageInfo_TriggerDescriptor_Event.DiscardUnknown(m)
}

var xxx_messageInfo_TriggerDescriptor_Event proto.InternalMessageInfo
...
type PrimaryKeySwap_LocalityConfigSwap struct {
	...
	// ColumnDescriptor.
	NewRegionalByRowColumnDefaultExpr *Expression `protobuf:"bytes,4,opt,name=new_regional_by_row_column_default_expr,json=newRegionalByRowColumnDefaultExpr,casttype=Expression" json:"new_regional_by_row_column_default_expr,omitempty"`
}
...
type ComputedColumnSwap struct {
	...
	InverseExpr Expression `protobuf:"bytes,3,opt,name=inverse_expr,json=inverseExpr,casttype=Expression" json:"inverse_expr"`
}
...
type PolicyDescriptor struct {
	...
	ID PolicyID `protobuf:"varint,1,opt,name=id,casttype=PolicyID" json:"id"`
	...
	Type catpb.PolicyType `protobuf:"varint,3,opt,name=type,enum=cockroach.sql.catalog.catpb.PolicyType" json:"type"`
	// Command specifies the SQL commands to which the policy applies.
	Command catpb.PolicyCommand `protobuf:"varint,4,opt,name=command,enum=cockroach.sql.catalog.catpb.PolicyCommand" json:"command"`
	// RoleNames is a list of roles that this policy applies to.
	RoleNames []string `protobuf:"bytes,5,rep,name=role_names,json=roleNames" json:"role_names,omitempty"`
	...
	UsingExpr Expression `protobuf:"bytes,6,opt,name=using_expr,json=usingExpr,casttype=Expression" json:"using_expr"`
	// UsingColumnIDs is an ordered list of column IDs used by the USING expression.
	UsingColumnIDs []ColumnID `protobuf:"varint,11,rep,name=using_column_ids,json=usingColumnIds,casttype=ColumnID" json:"using_column_ids,omitempty"`
	...
	WithCheckExpr Expression `protobuf:"bytes,7,opt,name=with_check_expr,json=withCheckExpr,casttype=Expression" json:"with_check_expr"`
	// WithCheckColumnIDs is an ordered list of column IDs used by the WITH CHECK
	// expression.
	WithCheckColumnIDs []ColumnID `protobuf:"varint,12,rep,name=with_check_column_ids,json=withCheckColumnIds,casttype=ColumnID" json:"with_check_column_ids,omitempty"`
	// DependsOnTypes are the IDs of all user defined types that the policy
	// expressions depend on.
	DependsOnTypes []ID `protobuf:"varint,8,rep,name=depends_on_types,json=dependsOnTypes,casttype=ID" json:"depends_on_types,omitempty"`
	// DependsOnFunctions are the IDs of the function that the policy expressions
	// depend on.
	DependsOnFunctions []ID `protobuf:"varint,9,rep,name=depends_on_functions,json=dependsOnFunctions,casttype=ID" json:"depends_on_functions,omitempty"`
	// DependsOnRelations are the IDs of the tables, views and sequences that the
	// policy expressions depend on.
	DependsOnRelations []ID `protobuf:"varint,10,rep,name=depends_on_relations,json=dependsOnRelations,casttype=ID" json:"depends_on_relations,omitempty"`
}

func (m *PolicyDescriptor) Reset()         { *m = PolicyDescriptor{} }
func (m *PolicyDescriptor) String() string { return proto.CompactTextString(m) }
func (*PolicyDescriptor) ProtoMessage()    {}
func (*PolicyDescriptor) Descriptor() ([]byte, []int) {
	return fileDescriptor_12dcc21c3bcc9571, []int{13}
}
func (m *PolicyDescriptor) XXX_Unmarshal(b []byte) error {
	return m.Unmarshal(b)
}
func (m *PolicyDescriptor) XXX_Marshal(b []byte, deterministic bool) ([]byte, error) {
	b = b[:cap(b)]
	n, err := m.MarshalToSizedBuffer(b)
	if err != nil {
		return nil, err
...
func (m *PolicyDescriptor) XXX_Merge(src proto.Message) {
	xxx_messageInfo_PolicyDescriptor.Merge(m, src)
}
func (m *PolicyDescriptor) XXX_Size() int {
	return m.Size()
}
func (m *PolicyDescriptor) XXX_DiscardUnknown() {
	xxx_messageInfo_PolicyDescriptor.DiscardUnknown(m)
}

var xxx_messageInfo_PolicyDescriptor proto.InternalMessageInfo
...
type TableDescriptor struct {
	...
	ViewQuery Statement `protobuf:"bytes,24,opt,name=view_query,json=viewQuery,casttype=Statement" json:"view_query"`
	...
	SecurityInvoker *bool `protobuf:"varint,72,opt,name=security_invoker,json=securityInvoker" json:"security_invoker,omitempty"`
	...
	CreateQuery Statement                                                    `protobuf:"bytes,34,opt,name=create_query,json=createQuery,casttype=Statement" json:"create_query"`
	...
	External *ExternalRowData `protobuf:"bytes,61,opt,name=external" json:"external,omitempty"`
	...
	LDRJobIDs []github_com_cockroachdb_cockroach_pkg_sql_catalog_catpb.JobID `protobuf:"varint,62,rep,name=ldr_job_ids,json=ldrJobIds,casttype=github.com/cockroachdb/cockroach/pkg/sql/catalog/catpb.JobID" json:"ldr_job_ids,omitempty"`
	// ReplicatedPCRVersion  tracks the original version from the source tenant
	// that this descriptor was created from.
	ReplicatedPCRVersion DescriptorVersion `protobuf:"varint,63,opt,name=replicated_pcr_version,json=replicatedPcrVersion,casttype=DescriptorVersion" json:"replicated_pcr_version"`
	// Triggers is the unordered list of triggers defined for this table.
	Triggers []TriggerDescriptor `protobuf:"bytes,64,rep,name=triggers" json:"triggers"`
	// Trigger ID for the next trigger.
	NextTriggerID TriggerID `protobuf:"varint,65,opt,name=next_trigger_id,json=nextTriggerId,casttype=TriggerID" json:"next_trigger_id"`
	// Policies are the row-level security policies that are defined on this table.
	Policies []PolicyDescriptor `protobuf:"bytes,66,rep,name=policies" json:"policies"`
	// NextPolicyID is the ID for the next policy created.
	NextPolicyID PolicyID `protobuf:"varint,67,opt,name=next_policy_id,json=nextPolicyId,casttype=PolicyID" json:"next_policy_id"`
	// row_level_security_enabled indicates whether row level security is enabled on this table.
	RowLevelSecurityEnabled bool `protobuf:"varint,68,opt,name=row_level_security_enabled,json=rowLevelSecurityEnabled" json:"row_level_security_enabled"`
	...
	RowLevelSecurityForced bool `protobuf:"varint,69,opt,name=row_level_security_forced,json=rowLevelSecurityForced" json:"row_level_security_forced"`
	...
	RBRUsingConstraint ConstraintID `protobuf:"varint,70,opt,name=rbr_using_constraint,json=rbrUsingConstraint,casttype=ConstraintID" json:"rbr_using_constraint"`
	...
	// cluster.
	StatsCanaryWindow time.Duration `protobuf:"varint,71,opt,name=stats_canary_window,json=statsCanaryWindow,casttype=time.Duration" json:"stats_canary_window"`
}
...
func (m *TableDescriptor) GetViewQuery() Statement {
	if m != nil {
		return m.ViewQuery
	}
	return ""
...
func (m *TableDescriptor) GetSecurityInvoker() bool {
	if m != nil && m.SecurityInvoker != nil {
		return *m.SecurityInvoker
	}
	return false
}
...
func (m *TableDescriptor) GetCreateQuery() Statement {
	if m != nil {
...
func (m *TableDescriptor) GetExternal() *ExternalRowData {
	if m != nil {
		return m.External
	}
	return nil
...
func (m *TableDescriptor) GetLDRJobIDs() []github_com_cockroachdb_cockroach_pkg_sql_catalog_catpb.JobID {
	if m != nil {
		return m.LDRJobIDs
	}
	return nil
...
func (m *TableDescriptor) GetReplicatedPCRVersion() DescriptorVersion {
	if m != nil {
		return m.ReplicatedPCRVersion
	}
	return 0
}

func (m *TableDescriptor) GetTriggers() []TriggerDescriptor {
	if m != nil {
		return m.Triggers
	}
	return nil
...
func (m *TableDescriptor) GetNextTriggerID() TriggerID {
	if m != nil {
...
func (m *TableDescriptor) GetPolicies() []PolicyDescriptor {
	if m != nil {
		return m.Policies
	}
	return nil
...
func (m *TableDescriptor) GetNextPolicyID() PolicyID {
	if m != nil {
...
func (m *TableDescriptor) GetRowLevelSecurityEnabled() bool {
	if m != nil {
		return m.RowLevelSecurityEnabled
	}
	return false
}

func (m *TableDescriptor) GetRowLevelSecurityForced() bool {
	if m != nil {
		return m.RowLevelSecurityForced
	}
	return false
}

func (m *TableDescriptor) GetRBRUsingConstraint() ConstraintID {
	if m != nil {
		return m.RBRUsingConstraint
	}
	return 0
}

func (m *TableDescriptor) GetStatsCanaryWindow() time.Duration {
	if m != nil {
		return m.StatsCanaryWindow
	}
	return 0
}
...
type TableDescriptor_CheckConstraint struct {
	...
	// functions.
	Expr     Expression         `protobuf:"bytes,1,opt,name=expr,casttype=Expression" json:"expr"`
	Name     string             `protobuf:"bytes,2,opt,name=name" json:"name"`
...
type TableDescriptor_Reference struct {
	...
	// TriggerID is set when this dependency originates from a trigger on the
	// dependent relation. A value of 0 indicates no trigger.
	TriggerID TriggerID `protobuf:"varint,5,opt,name=trigger_id,json=triggerId,casttype=TriggerID" json:"trigger_id"`
}
...
func (*TableDescriptor_Reference) Descriptor() ([]byte, []int) {
	return fileDescriptor_12dcc21c3bcc9571, []int{16, 1}
}
...
type TableDescriptor_SequenceOpts struct {
	...
	SessionCacheSize int64 `protobuf:"varint,7,opt,name=session_cache_size,json=sessionCacheSize" json:"session_cache_size"`
...
func (*TableDescriptor_SequenceOpts) Descriptor() ([]byte, []int) {
	return fileDescriptor_12dcc21c3bcc9571, []int{16, 3}
}
...
func (*TableDescriptor_SequenceOpts_SequenceOwner) Descriptor() ([]byte, []int) {
	return fileDescriptor_12dcc21c3bcc9571, []int{16, 3, 0}
}
...
func (*TableDescriptor_Replacement) Descriptor() ([]byte, []int) {
	return fileDescriptor_12dcc21c3bcc9571, []int{16, 4}
}
...
type ExternalRowData struct {
	AsOf     hlc.Timestamp    `protobuf:"bytes,1,opt,name=as_of,json=asOf" json:"as_of"`
...
func (m *ExternalRowData) Reset()         { *m = ExternalRowData{} }
func (m *ExternalRowData) String() string { return proto.CompactTextString(m) }
func (*ExternalRowData) ProtoMessage()    {}
func (*ExternalRowData) Descriptor() ([]byte, []int) {
	return fileDescriptor_12dcc21c3bcc9571, []int{17}
}
func (m *ExternalRowData) XXX_Unmarshal(b []byte) error {
	return m.Unmarshal(b)
}
func (m *ExternalRowData) XXX_Marshal(b []byte, deterministic bool) ([]byte, error) {
	b = b[:cap(b)]
	n, err := m.MarshalToSizedBuffer(b)
	if err != nil {
		return nil, err
...
func (m *ExternalRowData) XXX_Merge(src proto.Message) {
	xxx_messageInfo_ExternalRowData.Merge(m, src)
}
func (m *ExternalRowData) XXX_Size() int {
	return m.Size()
}
func (m *ExternalRowData) XXX_DiscardUnknown() {
	xxx_messageInfo_ExternalRowData.DiscardUnknown(m)
}

var xxx_messageInfo_ExternalRowData proto.InternalMessageInfo
...
type DatabaseDescriptor struct {
	...
	// ReplicatedPCRVersion tracks the original version from the source tenant
	// that this descriptor was created from.
	ReplicatedPCRVersion DescriptorVersion `protobuf:"varint,14,opt,name=replicated_pcr_version,json=replicatedPcrVersion,casttype=DescriptorVersion" json:"replicated_pcr_version"`
}
...
func (m *DatabaseDescriptor) GetReplicatedPCRVersion() DescriptorVersion {
	if m != nil {
		return m.ReplicatedPCRVersion
	}
	return 0
...
type SuperRegion struct {
	...
	// Overrides the database-level survival goal for this super region.
	// Only meaningful when has_survival_goal is true.
	SurvivalGoal SurvivalGoal `protobuf:"varint,3,opt,name=survival_goal,json=survivalGoal,enum=cockroach.sql.sqlbase.SurvivalGoal" json:"survival_goal"`
	// When true, survival_goal is an explicit override. When false (default),
	// the super region inherits the database-level survival goal. Needed because
	// SurvivalGoal's zero value (ZONE_FAILURE) is also a valid explicit setting.
	HasSurvivalGoal bool `protobuf:"varint,4,opt,name=has_survival_goal,json=hasSurvivalGoal" json:"has_survival_goal"`
}
...
//     config extension, then with the super_region[<super_region_name>] zone
//     config extension (if the primary region belongs to a super region), and
//     then with the regional_in[<primary_region>] zone config extension. In
//     doing so, REGIONAL [ IN PRIMARY REGION ] tables can continue to inherit
//     from the database level zone config.
...
//     config extension, then with the super_region[<super_region_name>] zone
//     config extension (if the affinity region belongs to a super region), and
//     then with the regional_in[<affinity_region>] zone config extension.
//  3. partition-level zone configs are extended first using the regional zone
//     config extension, then with the super_region[<super_region_name>] zone
//     config extension (if the partition region belongs to a super region), and
//     then with the regional_in[<partition_region>] zone config extension.
type ZoneConfigExtensions struct {
	...
	// SuperRegion extends the zone config for partitions and REGIONAL BY TABLE
	// tables whose affinity region belongs to the named super region. Applied
	// after the regional extension and before the regional_in extension.
	SuperRegion map[string]zonepb.ZoneConfig `protobuf:"bytes,4,rep,name=super_region,json=superRegion" json:"super_region" protobuf_key:"bytes,1,opt,name=key" protobuf_val:"bytes,2,opt,name=value"`
}
...
type TypeDescriptor struct {
	...
	Domain *TypeDescriptor_Domain `protobuf:"bytes,21,opt,name=domain" json:"domain,omitempty"`
	// ReplicatedPCRVersion tracks the original version from the source tenant
	// that this descriptor was created from.
	ReplicatedPCRVersion DescriptorVersion `protobuf:"varint,20,opt,name=replicated_pcr_version,json=replicatedPcrVersion,casttype=DescriptorVersion" json:"replicated_pcr_version"`
}
...
func (m *TypeDescriptor) GetDomain() *TypeDescriptor_Domain {
	if m != nil {
		return m.Domain
	}
	return nil
...
func (m *TypeDescriptor) GetReplicatedPCRVersion() DescriptorVersion {
	if m != nil {
		return m.ReplicatedPCRVersion
	}
	return 0
...
func (*TypeDescriptor_RegionConfig) Descriptor() ([]byte, []int) {
	return fileDescriptor_12dcc21c3bcc9571, []int{21, 1}
}
...
func (*TypeDescriptor_Composite) Descriptor() ([]byte, []int) {
	return fileDescriptor_12dcc21c3bcc9571, []int{21, 2}
}
...
func (*TypeDescriptor_Composite_CompositeElement) Descriptor() ([]byte, []int) {
	return fileDescriptor_12dcc21c3bcc9571, []int{21, 2, 0}
}
...
// Domain describes a domain type, which is a base type with optional
// constraints (CHECK, NOT NULL) and a default expression.
type TypeDescriptor_Domain struct {
	// BaseType is the underlying type this domain is built on.
	BaseType *types.T `protobuf:"bytes,1,opt,name=base_type,json=baseType" json:"base_type,omitempty"`
	// NotNullState describes the domain's NOT NULL constraint and its
	// enforcement state. NONE means no NOT NULL constraint.
	NotNullState TypeDescriptor_Domain_NotNullState `protobuf:"varint,2,opt,name=not_null_state,json=notNullState,enum=cockroach.sql.sqlbase.TypeDescriptor_Domain_NotNullState" json:"not_null_state"`
	// DefaultExpr is the default expression for the domain, serialized as a
	// string. Empty if no default is specified.
	DefaultExpr string `protobuf:"bytes,3,opt,name=default_expr,json=defaultExpr" json:"default_expr"`
	// CheckConstraints is the list of CHECK constraints on this domain.
	CheckConstraints []TypeDescriptor_Domain_CheckConstraint `protobuf:"bytes,4,rep,name=check_constraints,json=checkConstraints" json:"check_constraints"`
	// NotNullConstraintName is the name of the NOT NULL constraint. Empty when
	// NotNullState is NONE.
	NotNullConstraintName string `protobuf:"bytes,5,opt,name=not_null_constraint_name,json=notNullConstraintName" json:"not_null_constraint_name"`
	// NotNullConstraintID uniquely identifies the NOT NULL constraint within
	// the domain. Zero if there is no NOT NULL constraint.
	NotNullConstraintID ConstraintID `protobuf:"varint,6,opt,name=not_null_constraint_id,json=notNullConstraintId,casttype=ConstraintID" json:"not_null_constraint_id"`
	// NextConstraintID is the next available constraint ID for this domain.
	// It is bumped whenever a NOT NULL or CHECK constraint is added.
	NextConstraintID ConstraintID `protobuf:"varint,7,opt,name=next_constraint_id,json=nextConstraintId,casttype=ConstraintID" json:"next_constraint_id"`
}

func (m *TypeDescriptor_Domain) Reset()         { *m = TypeDescriptor_Domain{} }
func (m *TypeDescriptor_Domain) String() string { return proto.CompactTextString(m) }
func (*TypeDescriptor_Domain) ProtoMessage()    {}
func (*TypeDescriptor_Domain) Descriptor() ([]byte, []int) {
	return fileDescriptor_12dcc21c3bcc9571, []int{21, 3}
}
func (m *TypeDescriptor_Domain) XXX_Unmarshal(b []byte) error {
	return m.Unmarshal(b)
}
func (m *TypeDescriptor_Domain) XXX_Marshal(b []byte, deterministic bool) ([]byte, error) {
	b = b[:cap(b)]
	n, err := m.MarshalToSizedBuffer(b)
	if err != nil {
		return nil, err
...
func (m *TypeDescriptor_Domain) XXX_Merge(src proto.Message) {
	xxx_messageInfo_TypeDescriptor_Domain.Merge(m, src)
}
func (m *TypeDescriptor_Domain) XXX_Size() int {
	return m.Size()
}
func (m *TypeDescriptor_Domain) XXX_DiscardUnknown() {
	xxx_messageInfo_TypeDescriptor_Domain.DiscardUnknown(m)
}

var xxx_messageInfo_TypeDescriptor_Domain proto.InternalMessageInfo

// CheckConstraint represents a single CHECK constraint on the domain.
type TypeDescriptor_Domain_CheckConstraint struct {
	// Name is the constraint name. May be auto-generated.
	Name string `protobuf:"bytes,1,opt,name=name" json:"name"`
	...
	ConstraintID ConstraintID `protobuf:"varint,3,opt,name=constraint_id,json=constraintId,casttype=ConstraintID" json:"constraint_id"`
	// Validity indicates whether the constraint has been validated for all
	// existing data.
	Validity ConstraintValidity `protobuf:"varint,4,opt,name=validity,enum=cockroach.sql.sqlbase.ConstraintValidity" json:"validity"`
}

func (m *TypeDescriptor_Domain_CheckConstraint) Reset()         { *m = TypeDescriptor_Domain_CheckConstraint{} }
func (m *TypeDescriptor_Domain_CheckConstraint) String() string { return proto.CompactTextString(m) }
func (*TypeDescriptor_Domain_CheckConstraint) ProtoMessage()    {}
func (*TypeDescriptor_Domain_CheckConstraint) Descriptor() ([]byte, []int) {
	return fileDescriptor_12dcc21c3bcc9571, []int{21, 3, 0}
}
func (m *TypeDescriptor_Domain_CheckConstraint) XXX_Unmarshal(b []byte) error {
	return m.Unmarshal(b)
}
func (m *TypeDescriptor_Domain_CheckConstraint) XXX_Marshal(b []byte, deterministic bool) ([]byte, error) {
	b = b[:cap(b)]
	n, err := m.MarshalToSizedBuffer(b)
	if err != nil {
		return nil, err
...
func (m *TypeDescriptor_Domain_CheckConstraint) XXX_Merge(src proto.Message) {
	xxx_messageInfo_TypeDescriptor_Domain_CheckConstraint.Merge(m, src)
}
func (m *TypeDescriptor_Domain_CheckConstraint) XXX_Size() int {
	return m.Size()
}
func (m *TypeDescriptor_Domain_CheckConstraint) XXX_DiscardUnknown() {
	xxx_messageInfo_TypeDescriptor_Domain_CheckConstraint.DiscardUnknown(m)
}

var xxx_messageInfo_TypeDescriptor_Domain_CheckConstraint proto.InternalMessageInfo
...
func (*SchemaDescriptor) Descriptor() ([]byte, []int) {
	return fileDescriptor_12dcc21c3bcc9571, []int{22}
}
...
func (m *SchemaDescriptor) GetReplicatedPCRVersion() DescriptorVersion {
	if m != nil {
		return m.ReplicatedPCRVersion
	}
	return 0
...
func (*SchemaDescriptor_FunctionSignature) Descriptor() ([]byte, []int) {
	return fileDescriptor_12dcc21c3bcc9571, []int{22, 0}
}
...
func (*SchemaDescriptor_Function) Descriptor() ([]byte, []int) {
	return fileDescriptor_12dcc21c3bcc9571, []int{22, 1}
}
...
type FunctionDescriptor struct {
	...
	FunctionBody RoutineBody               `protobuf:"bytes,8,opt,name=function_body,json=functionBody,casttype=RoutineBody" json:"function_body"`
	...
	Security catpb.Function_Security `protobuf:"varint,23,opt,name=security,enum=cockroach.sql.catalog.catpb.Function_Security" json:"security"`
	// ReplicatedPCRVersion tracks the original version from the source tenant
	// that this descriptor was created from.
	ReplicatedPCRVersion DescriptorVersion `protobuf:"varint,24,opt,name=replicated_pcr_version,json=replicatedPcrVersion,casttype=DescriptorVersion" json:"replicated_pcr_version"`
}
...
func (*FunctionDescriptor) Descriptor() ([]byte, []int) {
	return fileDescriptor_12dcc21c3bcc9571, []int{23}
}
...
func (m *FunctionDescriptor) GetFunctionBody() RoutineBody {
	if m != nil {
...
func (m *FunctionDescriptor) GetSecurity() catpb.Function_Security {
	if m != nil {
		return m.Security
	}
	return catpb.Function_INVOKER
}

func (m *FunctionDescriptor) GetReplicatedPCRVersion() DescriptorVersion {
	if m != nil {
		return m.ReplicatedPCRVersion
	}
	return 0
...
func (*FunctionDescriptor_Parameter) Descriptor() ([]byte, []int) {
	return fileDescriptor_12dcc21c3bcc9571, []int{23, 0}
}
...
func (*FunctionDescriptor_ReturnType) Descriptor() ([]byte, []int) {
	return fileDescriptor_12dcc21c3bcc9571, []int{23, 1}
}
...
type FunctionDescriptor_Reference struct {
	...
	TriggerIDs    []TriggerID    `protobuf:"varint,5,rep,name=trigger_ids,json=triggerIds,casttype=TriggerID" json:"trigger_ids,omitempty"`
	PolicyIDs     []PolicyID     `protobuf:"varint,6,rep,name=policy_ids,json=policyIds,casttype=PolicyID" json:"policy_ids,omitempty"`
	// ViewQuery indicates that this function is referenced in the query of a
	// view.
	ViewQuery bool `protobuf:"varint,7,opt,name=view_query,json=viewQuery" json:"view_query"`
}
...
func (*FunctionDescriptor_Reference) Descriptor() ([]byte, []int) {
	return fileDescriptor_12dcc21c3bcc9571, []int{23, 2}
}
...
func (*Descriptor) Descriptor() ([]byte, []int) {
	return fileDescriptor_12dcc21c3bcc9571, []int{24}
}
...
type DescriptorUpdates struct {
	DescriptorIDs      []ID                `protobuf:"varint,1,rep,name=descriptor_ids,json=descriptorIds,casttype=ID" json:"descriptor_ids,omitempty"`
	DescriptorVersions []DescriptorVersion `protobuf:"varint,2,rep,name=descriptor_versions,json=descriptorVersions,casttype=DescriptorVersion" json:"descriptor_versions,omitempty"`
}

func (m *DescriptorUpdates) Reset()         { *m = DescriptorUpdates{} }
func (m *DescriptorUpdates) String() string { return proto.CompactTextString(m) }
func (*DescriptorUpdates) ProtoMessage()    {}
func (*DescriptorUpdates) Descriptor() ([]byte, []int) {
	return fileDescriptor_12dcc21c3bcc9571, []int{25}
}
func (m *DescriptorUpdates) XXX_Unmarshal(b []byte) error {
	return m.Unmarshal(b)
}
func (m *DescriptorUpdates) XXX_Marshal(b []byte, deterministic bool) ([]byte, error) {
	b = b[:cap(b)]
	n, err := m.MarshalToSizedBuffer(b)
	if err != nil {
		return nil, err
...
func (m *DescriptorUpdates) XXX_Merge(src proto.Message) {
	xxx_messageInfo_DescriptorUpdates.Merge(m, src)
}
func (m *DescriptorUpdates) XXX_Size() int {
	return m.Size()
}
func (m *DescriptorUpdates) XXX_DiscardUnknown() {
	xxx_messageInfo_DescriptorUpdates.DiscardUnknown(m)
}

var xxx_messageInfo_DescriptorUpdates proto.InternalMessageInfo

func (m *DescriptorUpdates) GetDescriptorIDs() []ID {
	if m != nil {
		return m.DescriptorIDs
	}
	return nil
}

func (m *DescriptorUpdates) GetDescriptorVersions() []DescriptorVersion {
	if m != nil {
		return m.DescriptorVersions
	}
	return nil
}
...
func init() {
	...
	proto.RegisterEnum("cockroach.sql.sqlbase.TypeDescriptor_Domain_NotNullState", TypeDescriptor_Domain_NotNullState_name, TypeDescriptor_Domain_NotNullState_value)
	...
	proto.RegisterType((*TriggerDescriptor)(nil), "cockroach.sql.sqlbase.TriggerDescriptor")
	proto.RegisterType((*TriggerDescriptor_Event)(nil), "cockroach.sql.sqlbase.TriggerDescriptor.Event")
	...
	proto.RegisterType((*PolicyDescriptor)(nil), "cockroach.sql.sqlbase.PolicyDescriptor")
	...
	proto.RegisterType((*ExternalRowData)(nil), "cockroach.sql.sqlbase.ExternalRowData")
	...
	proto.RegisterMapType((map[string]zonepb.ZoneConfig)(nil), "cockroach.sql.sqlbase.ZoneConfigExtensions.SuperRegionEntry")
	...
	proto.RegisterType((*TypeDescriptor_Domain)(nil), "cockroach.sql.sqlbase.TypeDescriptor.Domain")
	proto.RegisterType((*TypeDescriptor_Domain_CheckConstraint)(nil), "cockroach.sql.sqlbase.TypeDescriptor.Domain.CheckConstraint")
	...
	proto.RegisterType((*DescriptorUpdates)(nil), "cockroach.sql.sqlbase.DescriptorUpdates")
}
...
var fileDescriptor_12dcc21c3bcc9571 = []byte{
	...
	,
}
...
func (this *IndexDescriptor) Equal(that interface{}) bool {
	...
	if this.HideForPrimaryKeyRecreate != that1.HideForPrimaryKeyRecreate {
		return false
	}
	if this.SkipUniqueChecks != that1.SkipUniqueChecks {
		return false
	}
	return true
}
func (this *TriggerDescriptor) Equal(that interface{}) bool {
	if that == nil {
		return this == nil
	}

	that1, ok := that.(*TriggerDescriptor)
	if !ok {
		that2, ok := that.(TriggerDescriptor)
		if ok {
			that1 = &that2
		} else {
			return false
	...
	if len(this.DependsOnRoutines) != len(that1.DependsOnRoutines) {
		return false
	}
	for i := range this.DependsOnRoutines {
		if this.DependsOnRoutines[i] != that1.DependsOnRoutines[i] {
			return false
		}
	}
	return true
}
func (this *TriggerDescriptor_Event) Equal(that interface{}) bool {
	if that == nil {
		return this == nil
	}

	that1, ok := that.(*TriggerDescriptor_Event)
	if !ok {
		that2, ok := that.(TriggerDescriptor_Event)
		if ok {
			that1 = &that2
		} else {
			return false
...
func (this *PolicyDescriptor) Equal(that interface{}) bool {
	if that == nil {
		return this == nil
	}

	that1, ok := that.(*PolicyDescriptor)
	if !ok {
		that2, ok := that.(PolicyDescriptor)
		if ok {
			that1 = &that2
		} else {
			return false
	...
	if len(this.UsingColumnIDs) != len(that1.UsingColumnIDs) {
		return false
	}
	for i := range this.UsingColumnIDs {
		if this.UsingColumnIDs[i] != that1.UsingColumnIDs[i] {
			return false
		}
	}
	if this.WithCheckExpr != that1.WithCheckExpr {
		return false
	}
	if len(this.WithCheckColumnIDs) != len(that1.WithCheckColumnIDs) {
		return false
	}
	for i := range this.WithCheckColumnIDs {
		if this.WithCheckColumnIDs[i] != that1.WithCheckColumnIDs[i] {
			return false
		}
	}
	if len(this.DependsOnTypes) != len(that1.DependsOnTypes) {
	...
	if len(this.DependsOnRelations) != len(that1.DependsOnRelations) {
		return false
	}
	for i := range this.DependsOnRelations {
		if this.DependsOnRelations[i] != that1.DependsOnRelations[i] {
			return false
		}
	}
	return true
...
func (this *TableDescriptor) Equal(that interface{}) bool {
	...
	if this.SecurityInvoker != nil && that1.SecurityInvoker != nil {
		if *this.SecurityInvoker != *that1.SecurityInvoker {
			return false
		}
	} else if this.SecurityInvoker != nil {
		return false
	} else if that1.SecurityInvoker != nil {
		return false
	}
	if len(this.DependsOn) != len(that1.DependsOn) {
		return false
	...
	if len(this.LDRJobIDs) != len(that1.LDRJobIDs) {
		return false
	}
	for i := range this.LDRJobIDs {
		if this.LDRJobIDs[i] != that1.LDRJobIDs[i] {
			return false
		}
	}
	if this.ReplicatedPCRVersion != that1.ReplicatedPCRVersion {
		return false
	}
	...
	if this.NextTriggerID != that1.NextTriggerID {
		return false
	}
	...
	if this.NextPolicyID != that1.NextPolicyID {
		return false
	}
	if this.RowLevelSecurityEnabled != that1.RowLevelSecurityEnabled {
		return false
	}
	if this.RowLevelSecurityForced != that1.RowLevelSecurityForced {
		return false
	}
	if this.RBRUsingConstraint != that1.RBRUsingConstraint {
		return false
	}
	if this.StatsCanaryWindow != that1.StatsCanaryWindow {
		return false
	}
	return true
}
...
func (this *TableDescriptor_SequenceOpts) Equal(that interface{}) bool {
	...
	if this.SessionCacheSize != that1.SessionCacheSize {
		return false
	}
...
func (this *ExternalRowData) Equal(that interface{}) bool {
	if that == nil {
		return this == nil
	}

	that1, ok := that.(*ExternalRowData)
	if !ok {
		that2, ok := that.(ExternalRowData)
		if ok {
			that1 = &that2
		} else {
			return false
...
func (this *SuperRegion) Equal(that interface{}) bool {
	...
	if this.HasSurvivalGoal != that1.HasSurvivalGoal {
		return false
	}
	return true
}
func (this *ZoneConfigExtensions) Equal(that interface{}) bool {
	...
	if len(this.SuperRegion) != len(that1.SuperRegion) {
		return false
	}
	for i := range this.SuperRegion {
		a := this.SuperRegion[i]
		b := that1.SuperRegion[i]
		if !(&a).Equal(&b) {
			return false
		}
	}
...
func (this *TypeDescriptor_Domain) Equal(that interface{}) bool {
	if that == nil {
		return this == nil
	}

	that1, ok := that.(*TypeDescriptor_Domain)
	if !ok {
		that2, ok := that.(TypeDescriptor_Domain)
		if ok {
			that1 = &that2
		} else {
			return false
	...
	if !this.BaseType.Equal(that1.BaseType) {
		return false
	}
	if this.NotNullState != that1.NotNullState {
		return false
	}
	if this.DefaultExpr != that1.DefaultExpr {
		return false
	}
	if len(this.CheckConstraints) != len(that1.CheckConstraints) {
		return false
	}
	for i := range this.CheckConstraints {
		if !this.CheckConstraints[i].Equal(&that1.CheckConstraints[i]) {
			return false
		}
	}
	if this.NotNullConstraintName != that1.NotNullConstraintName {
		return false
	}
	if this.NotNullConstraintID != that1.NotNullConstraintID {
		return false
	}
	if this.NextConstraintID != that1.NextConstraintID {
		return false
...
func (this *TypeDescriptor_Domain_CheckConstraint) Equal(that interface{}) bool {
	if that == nil {
		return this == nil
	}

	that1, ok := that.(*TypeDescriptor_Domain_CheckConstraint)
	if !ok {
		that2, ok := that.(TypeDescriptor_Domain_CheckConstraint)
		if ok {
			that1 = &that2
		} else {
			return false
...
func (this *FunctionDescriptor_Reference) Equal(that interface{}) bool {
	...
	if len(this.TriggerIDs) != len(that1.TriggerIDs) {
		return false
	}
	for i := range this.TriggerIDs {
		if this.TriggerIDs[i] != that1.TriggerIDs[i] {
			return false
		}
	}
	if len(this.PolicyIDs) != len(that1.PolicyIDs) {
		return false
	}
	for i := range this.PolicyIDs {
		if this.PolicyIDs[i] != that1.PolicyIDs[i] {
			return false
		}
	}
	if this.ViewQuery != that1.ViewQuery {
...
func (this *DescriptorUpdates) Equal(that interface{}) bool {
	if that == nil {
		return this == nil
	}

	that1, ok := that.(*DescriptorUpdates)
	if !ok {
		that2, ok := that.(DescriptorUpdates)
		if ok {
			that1 = &that2
		} else {
			return false
	...
	if len(this.DescriptorIDs) != len(that1.DescriptorIDs) {
		return false
	}
	for i := range this.DescriptorIDs {
		if this.DescriptorIDs[i] != that1.DescriptorIDs[i] {
			return false
		}
	}
	if len(this.DescriptorVersions) != len(that1.DescriptorVersions) {
		return false
	}
	for i := range this.DescriptorVersions {
		if this.DescriptorVersions[i] != that1.DescriptorVersions[i] {
			return false
		}
	}
	return true
...
func (m *IndexDescriptor) MarshalToSizedBuffer(dAtA []byte) (int, error) {
	...
	dAtA[i] = 0x80
	i--
	if m.HideForPrimaryKeyRecreate {
		dAtA[i] = 1
	} else {
		dAtA[i] = 0
	}
...
func (m *TriggerDescriptor) Marshal() (dAtA []byte, err error) {
	size := m.Size()
	dAtA = make([]byte, size)
	n, err := m.MarshalToSizedBuffer(dAtA[:size])
	if err != nil {
...
func (m *TriggerDescriptor) MarshalTo(dAtA []byte) (int, error) {
	size := m.Size()
	return m.MarshalToSizedBuffer(dAtA[:size])
}

func (m *TriggerDescriptor) MarshalToSizedBuffer(dAtA []byte) (int, error) {
	i := len(dAtA)
	_ = i
	var l int
	_ = l
	if len(m.DependsOnRoutines) > 0 {
		for iNdEx := len(m.DependsOnRoutines) - 1; iNdEx >= 0; iNdEx-- {
			i = encodeVarintStructured(dAtA, i, uint64(m.DependsOnRoutines[iNdEx]))
			i--
			dAtA[i] = 0x78
		}
	}
	...
	i = encodeVarintStructured(dAtA, i, uint64(len(m.FuncBody)))
	i--
	dAtA[i] = 0x5a
	if len(m.FuncArgs) > 0 {
		for iNdEx := len(m.FuncArgs) - 1; iNdEx >= 0; iNdEx-- {
			...
			i = encodeVarintStructured(dAtA, i, uint64(len(m.FuncArgs[iNdEx])))
			i--
			dAtA[i] = 0x52
		}
	}
	i = encodeVarintStructured(dAtA, i, uint64(m.FuncID))
	i--
	dAtA[i] = 0x48
	i -= len(m.WhenExpr)
	copy(dAtA[i:], m.WhenExpr)
	i = encodeVarintStructured(dAtA, i, uint64(len(m.WhenExpr)))
	i--
	dAtA[i] = 0x42
	i--
	if m.ForEachRow {
	...
	i = encodeVarintStructured(dAtA, i, uint64(len(m.OldTransitionAlias)))
	i--
	dAtA[i] = 0x32
	i -= len(m.NewTransitionAlias)
	copy(dAtA[i:], m.NewTransitionAlias)
	i = encodeVarintStructured(dAtA, i, uint64(len(m.NewTransitionAlias)))
	i--
	dAtA[i] = 0x2a
	if len(m.Events) > 0 {
	...
	i = encodeVarintStructured(dAtA, i, uint64(m.ActionTime))
	i--
	dAtA[i] = 0x18
	i -= len(m.Name)
	copy(dAtA[i:], m.Name)
...
func (m *TriggerDescriptor_Event) Marshal() (dAtA []byte, err error) {
	size := m.Size()
	dAtA = make([]byte, size)
	n, err := m.MarshalToSizedBuffer(dAtA[:size])
	if err != nil {
...
func (m *TriggerDescriptor_Event) MarshalTo(dAtA []byte) (int, error) {
	size := m.Size()
	return m.MarshalToSizedBuffer(dAtA[:size])
}

func (m *TriggerDescriptor_Event) MarshalToSizedBuffer(dAtA []byte) (int, error) {
	i := len(dAtA)
	_ = i
	var l int
	_ = l
...
func (m *PolicyDescriptor) Marshal() (dAtA []byte, err error) {
	size := m.Size()
	dAtA = make([]byte, size)
	n, err := m.MarshalToSizedBuffer(dAtA[:size])
	if err != nil {
...
func (m *PolicyDescriptor) MarshalTo(dAtA []byte) (int, error) {
	size := m.Size()
	return m.MarshalToSizedBuffer(dAtA[:size])
}

func (m *PolicyDescriptor) MarshalToSizedBuffer(dAtA []byte) (int, error) {
	i := len(dAtA)
	_ = i
	var l int
	_ = l
	if len(m.WithCheckColumnIDs) > 0 {
		for iNdEx := len(m.WithCheckColumnIDs) - 1; iNdEx >= 0; iNdEx-- {
			i = encodeVarintStructured(dAtA, i, uint64(m.WithCheckColumnIDs[iNdEx]))
			i--
			dAtA[i] = 0x60
		}
	}
	if len(m.UsingColumnIDs) > 0 {
		for iNdEx := len(m.UsingColumnIDs) - 1; iNdEx >= 0; iNdEx-- {
			i = encodeVarintStructured(dAtA, i, uint64(m.UsingColumnIDs[iNdEx]))
			i--
			dAtA[i] = 0x58
		}
	}
	if len(m.DependsOnRelations) > 0 {
		for iNdEx := len(m.DependsOnRelations) - 1; iNdEx >= 0; iNdEx-- {
			i = encodeVarintStructured(dAtA, i, uint64(m.DependsOnRelations[iNdEx]))
			i--
			dAtA[i] = 0x50
		}
	}
	...
	i -= len(m.WithCheckExpr)
	copy(dAtA[i:], m.WithCheckExpr)
	i = encodeVarintStructured(dAtA, i, uint64(len(m.WithCheckExpr)))
	i--
	dAtA[i] = 0x3a
	i -= len(m.UsingExpr)
	copy(dAtA[i:], m.UsingExpr)
	i = encodeVarintStructured(dAtA, i, uint64(len(m.UsingExpr)))
	i--
	dAtA[i] = 0x32
	if len(m.RoleNames) > 0 {
		for iNdEx := len(m.RoleNames) - 1; iNdEx >= 0; iNdEx-- {
			...
			i = encodeVarintStructured(dAtA, i, uint64(len(m.RoleNames[iNdEx])))
			i--
			dAtA[i] = 0x2a
		}
	}
	i = encodeVarintStructured(dAtA, i, uint64(m.Command))
	i--
	dAtA[i] = 0x20
	i = encodeVarintStructured(dAtA, i, uint64(m.Type))
	i--
...
func (m *TableDescriptor) MarshalToSizedBuffer(dAtA []byte) (int, error) {
	...
	var l int
	_ = l
	if m.SecurityInvoker != nil {
		i--
		if *m.SecurityInvoker {
			dAtA[i] = 1
		} else {
			dAtA[i] = 0
		}
	...
	i = encodeVarintStructured(dAtA, i, uint64(m.StatsCanaryWindow))
	i--
	dAtA[i] = 0x4
	i--
	dAtA[i] = 0xb8
	i = encodeVarintStructured(dAtA, i, uint64(m.RBRUsingConstraint))
	i--
	dAtA[i] = 0x4
	i--
	dAtA[i] = 0xb0
	i--
	if m.RowLevelSecurityForced {
		dAtA[i] = 1
	} else {
		dAtA[i] = 0
	}
	...
	i--
	dAtA[i] = 0xa0
	i = encodeVarintStructured(dAtA, i, uint64(m.NextPolicyID))
	i--
	dAtA[i] = 0x4
	i--
	dAtA[i] = 0x98
	...
	i = encodeVarintStructured(dAtA, i, uint64(m.NextTriggerID))
	i--
	dAtA[i] = 0x4
	i--
	dAtA[i] = 0x88
	...
	i = encodeVarintStructured(dAtA, i, uint64(m.ReplicatedPCRVersion))
	i--
	dAtA[i] = 0x3
	i--
	dAtA[i] = 0xf8
	if len(m.LDRJobIDs) > 0 {
		for iNdEx := len(m.LDRJobIDs) - 1; iNdEx >= 0; iNdEx-- {
			i = encodeVarintStructured(dAtA, i, uint64(m.LDRJobIDs[iNdEx]))
			i--
			dAtA[i] = 0x3
			i--
			dAtA[i] = 0xf0
...
func (m *TableDescriptor_Reference) MarshalToSizedBuffer(dAtA []byte) (int, error) {
	...
	var l int
	_ = l
	i = encodeVarintStructured(dAtA, i, uint64(m.TriggerID))
	i--
	dAtA[i] = 0x28
	i--
	if m.ByID {
...
func (m *TableDescriptor_SequenceOpts) MarshalToSizedBuffer(dAtA []byte) (int, error) {
	...
	i--
	dAtA[i] = 0x42
	i = encodeVarintStructured(dAtA, i, uint64(m.SessionCacheSize))
	i--
	dAtA[i] = 0x38
	{
...
func (m *ExternalRowData) Marshal() (dAtA []byte, err error) {
	size := m.Size()
	dAtA = make([]byte, size)
	n, err := m.MarshalToSizedBuffer(dAtA[:size])
	if err != nil {
...
func (m *ExternalRowData) MarshalTo(dAtA []byte) (int, error) {
	size := m.Size()
	return m.MarshalToSizedBuffer(dAtA[:size])
}

func (m *ExternalRowData) MarshalToSizedBuffer(dAtA []byte) (int, error) {
	i := len(dAtA)
	_ = i
	var l int
	_ = l
...
func (m *ZoneConfigExtensions) MarshalToSizedBuffer(dAtA []byte) (int, error) {
	...
	var l int
	_ = l
	if len(m.SuperRegion) > 0 {
		keysForSuperRegion := make([]string, 0, len(m.SuperRegion))
		for k := range m.SuperRegion {
			keysForSuperRegion = append(keysForSuperRegion, string(k))
		}
		github_com_gogo_protobuf_sortkeys.Strings(keysForSuperRegion)
		for iNdEx := len(keysForSuperRegion) - 1; iNdEx >= 0; iNdEx-- {
			v := m.SuperRegion[string(keysForSuperRegion[iNdEx])]
			baseI := i
			{
				size, err := (&v).MarshalToSizedBuffer(dAtA[:i])
				if err != nil {
			...
			i--
			dAtA[i] = 0x12
			i -= len(keysForSuperRegion[iNdEx])
			copy(dAtA[i:], keysForSuperRegion[iNdEx])
			i = encodeVarintStructured(dAtA, i, uint64(len(keysForSuperRegion[iNdEx])))
			i--
			dAtA[i] = 0xa
			i = encodeVarintStructured(dAtA, i, uint64(baseI-i))
			i--
...
func (m *TypeDescriptor_Domain) Marshal() (dAtA []byte, err error) {
	size := m.Size()
	dAtA = make([]byte, size)
	n, err := m.MarshalToSizedBuffer(dAtA[:size])
	if err != nil {
...
func (m *TypeDescriptor_Domain) MarshalTo(dAtA []byte) (int, error) {
	size := m.Size()
	return m.MarshalToSizedBuffer(dAtA[:size])
}

func (m *TypeDescriptor_Domain) MarshalToSizedBuffer(dAtA []byte) (int, error) {
	i := len(dAtA)
	_ = i
	var l int
	_ = l
	...
	i--
	dAtA[i] = 0x38
	i = encodeVarintStructured(dAtA, i, uint64(m.NotNullConstraintID))
	i--
	dAtA[i] = 0x30
	i -= len(m.NotNullConstraintName)
	copy(dAtA[i:], m.NotNullConstraintName)
	i = encodeVarintStructured(dAtA, i, uint64(len(m.NotNullConstraintName)))
	i--
	dAtA[i] = 0x2a
	if len(m.CheckConstraints) > 0 {
		for iNdEx := len(m.CheckConstraints) - 1; iNdEx >= 0; iNdEx-- {
			{
				size, err := m.CheckConstraints[iNdEx].MarshalToSizedBuffer(dAtA[:i])
				if err != nil {
					return 0, err
				}
				i -= size
	...
	i--
	dAtA[i] = 0x1a
	i = encodeVarintStructured(dAtA, i, uint64(m.NotNullState))
	i--
	dAtA[i] = 0x10
	if m.BaseType != nil {
		{
...
func (m *TypeDescriptor_Domain_CheckConstraint) Marshal() (dAtA []byte, err error) {
	size := m.Size()
	dAtA = make([]byte, size)
	n, err := m.MarshalToSizedBuffer(dAtA[:size])
	if err != nil {
...
func (m *TypeDescriptor_Domain_CheckConstraint) MarshalTo(dAtA []byte) (int, error) {
	size := m.Size()
	return m.MarshalToSizedBuffer(dAtA[:size])
}

func (m *TypeDescriptor_Domain_CheckConstraint) MarshalToSizedBuffer(dAtA []byte) (int, error) {
	i := len(dAtA)
	_ = i
	var l int
	_ = l
...
func (m *FunctionDescriptor) MarshalToSizedBuffer(dAtA []byte) (int, error) {
	...
	i--
	dAtA[i] = 0xc0
	i = encodeVarintStructured(dAtA, i, uint64(m.Security))
	i--
	dAtA[i] = 0x1
	i--
	dAtA[i] = 0xb8
...
func (m *FunctionDescriptor_Reference) MarshalToSizedBuffer(dAtA []byte) (int, error) {
	...
	if len(m.PolicyIDs) > 0 {
		for iNdEx := len(m.PolicyIDs) - 1; iNdEx >= 0; iNdEx-- {
			i = encodeVarintStructured(dAtA, i, uint64(m.PolicyIDs[iNdEx]))
			i--
			dAtA[i] = 0x30
		}
	}
	if len(m.TriggerIDs) > 0 {
		for iNdEx := len(m.TriggerIDs) - 1; iNdEx >= 0; iNdEx-- {
			i = encodeVarintStructured(dAtA, i, uint64(m.TriggerIDs[iNdEx]))
			i--
			dAtA[i] = 0x28
		}
	}
...
func (m *DescriptorUpdates) Marshal() (dAtA []byte, err error) {
	size := m.Size()
	dAtA = make([]byte, size)
	n, err := m.MarshalToSizedBuffer(dAtA[:size])
	if err != nil {
...
func (m *DescriptorUpdates) MarshalTo(dAtA []byte) (int, error) {
	size := m.Size()
	return m.MarshalToSizedBuffer(dAtA[:size])
}

func (m *DescriptorUpdates) MarshalToSizedBuffer(dAtA []byte) (int, error) {
	i := len(dAtA)
	_ = i
	var l int
	_ = l
	if len(m.DescriptorVersions) > 0 {
		for iNdEx := len(m.DescriptorVersions) - 1; iNdEx >= 0; iNdEx-- {
			i = encodeVarintStructured(dAtA, i, uint64(m.DescriptorVersions[iNdEx]))
			i--
			dAtA[i] = 0x10
		}
	}
	if len(m.DescriptorIDs) > 0 {
		for iNdEx := len(m.DescriptorIDs) - 1; iNdEx >= 0; iNdEx-- {
			i = encodeVarintStructured(dAtA, i, uint64(m.DescriptorIDs[iNdEx]))
			i--
			dAtA[i] = 0x8
		}
	}
...
func (m *TriggerDescriptor) Size() (n int) {
	if m == nil {
		return 0
	}
	var l int
	...
	l = len(m.Name)
	n += 1 + l + sovStructured(uint64(l))
	n += 1 + sovStructured(uint64(m.ActionTime))
	if len(m.Events) > 0 {
	...
	l = len(m.WhenExpr)
	n += 1 + l + sovStructured(uint64(l))
	n += 1 + sovStructured(uint64(m.FuncID))
	if len(m.FuncArgs) > 0 {
	...
	if len(m.DependsOnRoutines) > 0 {
		for _, e := range m.DependsOnRoutines {
			n += 1 + sovStructured(uint64(e))
		}
	}
	return n
...
func (m *TriggerDescriptor_Event) Size() (n int) {
	if m == nil {
		return 0
	}
	var l int
...
func (m *PolicyDescriptor) Size() (n int) {
	if m == nil {
		return 0
	}
	var l int
	...
	n += 1 + l + sovStructured(uint64(l))
	n += 1 + sovStructured(uint64(m.Type))
	n += 1 + sovStructured(uint64(m.Command))
	if len(m.RoleNames) > 0 {
	...
	if len(m.DependsOnRelations) > 0 {
		for _, e := range m.DependsOnRelations {
			n += 1 + sovStructured(uint64(e))
		}
	}
	if len(m.UsingColumnIDs) > 0 {
		for _, e := range m.UsingColumnIDs {
			n += 1 + sovStructured(uint64(e))
		}
	}
	if len(m.WithCheckColumnIDs) > 0 {
		for _, e := range m.WithCheckColumnIDs {
			n += 1 + sovStructured(uint64(e))
		}
	}
	return n
...
func (m *TableDescriptor) Size() (n int) {
	...
	if len(m.LDRJobIDs) > 0 {
		for _, e := range m.LDRJobIDs {
			n += 2 + sovStructured(uint64(e))
		}
	}
	n += 2 + sovStructured(uint64(m.ReplicatedPCRVersion))
	if len(m.Triggers) > 0 {
	...
	n += 2 + sovStructured(uint64(m.NextTriggerID))
	if len(m.Policies) > 0 {
	...
	n += 2 + sovStructured(uint64(m.NextPolicyID))
	n += 3
	n += 3
	n += 2 + sovStructured(uint64(m.RBRUsingConstraint))
	n += 2 + sovStructured(uint64(m.StatsCanaryWindow))
	if m.SecurityInvoker != nil {
		n += 3
	}
	return n
...
func (m *TableDescriptor_Reference) Size() (n int) {
	...
	n += 2
	n += 1 + sovStructured(uint64(m.TriggerID))
	return n
}
...
func (m *TableDescriptor_SequenceOpts) Size() (n int) {
	...
	n += 1 + l + sovStructured(uint64(l))
	n += 1 + sovStructured(uint64(m.SessionCacheSize))
...
func (m *ExternalRowData) Size() (n int) {
	if m == nil {
		return 0
	}
	var l int
...
func (m *DatabaseDescriptor) Size() (n int) {
	...
	n += 1 + sovStructured(uint64(m.ReplicatedPCRVersion))
	return n
}
...
func (m *TypeDescriptor_Domain) Size() (n int) {
	if m == nil {
		return 0
	}
	var l int
	...
	n += 1 + sovStructured(uint64(m.NotNullState))
	l = len(m.DefaultExpr)
	n += 1 + l + sovStructured(uint64(l))
	if len(m.CheckConstraints) > 0 {
		for _, e := range m.CheckConstraints {
			l = e.Size()
			n += 1 + l + sovStructured(uint64(l))
		}
	}
	l = len(m.NotNullConstraintName)
	n += 1 + l + sovStructured(uint64(l))
	n += 1 + sovStructured(uint64(m.NotNullConstraintID))
	n += 1 + sovStructured(uint64(m.NextConstraintID))
	return n
}

func (m *TypeDescriptor_Domain_CheckConstraint) Size() (n int) {
	if m == nil {
		return 0
	}
	var l int
...
func (m *FunctionDescriptor) Size() (n int) {
	...
	n += 2 + sovStructured(uint64(m.Security))
	n += 2 + sovStructured(uint64(m.ReplicatedPCRVersion))
	return n
}
...
func (m *FunctionDescriptor_Reference) Size() (n int) {
	...
	if len(m.TriggerIDs) > 0 {
		for _, e := range m.TriggerIDs {
			n += 1 + sovStructured(uint64(e))
		}
	}
	if len(m.PolicyIDs) > 0 {
		for _, e := range m.PolicyIDs {
			n += 1 + sovStructured(uint64(e))
		}
	}
	n += 2
...
func (m *DescriptorUpdates) Size() (n int) {
	if m == nil {
		return 0
	}
	var l int
	...
	if len(m.DescriptorVersions) > 0 {
		for _, e := range m.DescriptorVersions {
			n += 1 + sovStructured(uint64(e))
		}
	}
	return n
	...
	for iNdEx < l {
		...
		case 2:
			if wireType == 0 {
				...
				elementCount = count
				if elementCount != 0 {
					if m.OriginColumnIDs == nil {
						m.OriginColumnIDs = make([]ColumnID, 0, elementCount)
					} else {
						m.OriginColumnIDs = slices.Grow(m.OriginColumnIDs, elementCount)
					}
				}
				for iNdEx < postIndex {
					var v ColumnID
...
func (m *UniqueWithoutIndexConstraint) Unmarshal(dAtA []byte) error {
	...
	for iNdEx < l {
		...
		case 5:
			...
			m.Predicate = Expression(dAtA[iNdEx:postIndex])
			iNdEx = postIndex
		case 6:
			if wireType != 0 {
				return fmt.Errorf("proto: wrong wireType = %d for field ConstraintID", wireType)
...
func (m *ColumnDescriptor) Unmarshal(dAtA []byte) error {
	...
	for iNdEx < l {
		...
		case 5:
			...
			s := Expression(dAtA[iNdEx:postIndex])
			m.DefaultExpr = &s
			iNdEx = postIndex
		case 6:
			if wireType != 0 {
		...
		case 12:
			if wireType == 0 {
				...
				elementCount = count
				if elementCount != 0 {
					if m.OwnsSequenceIds == nil {
						m.OwnsSequenceIds = make([]ID, 0, elementCount)
					} else {
						m.OwnsSequenceIds = slices.Grow(m.OwnsSequenceIds, elementCount)
					}
				}
				for iNdEx < postIndex {
					var v ID
	...
	for iNdEx < l {
		...
		case 6:
			if wireType == 0 {
				...
				elementCount = count
				if elementCount != 0 {
					if m.KeyColumnIDs == nil {
						m.KeyColumnIDs = make([]ColumnID, 0, elementCount)
					} else {
						m.KeyColumnIDs = slices.Grow(m.KeyColumnIDs, elementCount)
					}
				}
				for iNdEx < postIndex {
					var v ColumnID
		...
		case 7:
			if wireType == 0 {
				...
				elementCount = count
				if elementCount != 0 {
					if m.KeySuffixColumnIDs == nil {
						m.KeySuffixColumnIDs = make([]ColumnID, 0, elementCount)
					} else {
						m.KeySuffixColumnIDs = slices.Grow(m.KeySuffixColumnIDs, elementCount)
					}
				}
				for iNdEx < postIndex {
					var v ColumnID
		...
		case 8:
			if wireType == 0 {
				...
				var elementCount int
				if elementCount != 0 {
					if m.KeyColumnDirections == nil {
						m.KeyColumnDirections = make([]catenumpb.IndexColumn_Direction, 0, elementCount)
					} else {
						m.KeyColumnDirections = slices.Grow(m.KeyColumnDirections, elementCount)
					}
				}
				for iNdEx < postIndex {
					var v catenumpb.IndexColumn_Direction
		...
		case 13:
			if wireType == 0 {
				...
				elementCount = count
				if elementCount != 0 {
					if m.CompositeColumnIDs == nil {
						m.CompositeColumnIDs = make([]ColumnID, 0, elementCount)
					} else {
						m.CompositeColumnIDs = slices.Grow(m.CompositeColumnIDs, elementCount)
					}
				}
				for iNdEx < postIndex {
					var v ColumnID
		...
		case 14:
			if wireType == 0 {
				...
				elementCount = count
				if elementCount != 0 {
					if m.StoreColumnIDs == nil {
						m.StoreColumnIDs = make([]ColumnID, 0, elementCount)
					} else {
						m.StoreColumnIDs = slices.Grow(m.StoreColumnIDs, elementCount)
					}
				}
				for iNdEx < postIndex {
					var v ColumnID
		...
		case 27:
			if wireType == 0 {
				...
				var elementCount int
				if elementCount != 0 {
					if m.InvertedColumnKinds == nil {
						m.InvertedColumnKinds = make([]catpb.InvertedIndexColumnKind, 0, elementCount)
					} else {
						m.InvertedColumnKinds = slices.Grow(m.InvertedColumnKinds, elementCount)
					}
				}
				for iNdEx < postIndex {
					var v catpb.InvertedIndexColumnKind
		...
		case 31:
			if wireType != 0 {
				return fmt.Errorf("proto: wrong wireType = %d for field HideForPrimaryKeyRecreate", wireType)
			}
			var v int
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
			...
			m.HideForPrimaryKeyRecreate = bool(v != 0)
		case 32:
			if wireType != 0 {
				return fmt.Errorf("proto: wrong wireType = %d for field SkipUniqueChecks", wireType)
			}
			var v int
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
...
func (m *TriggerDescriptor) Unmarshal(dAtA []byte) error {
	l := len(dAtA)
	iNdEx := 0
	for iNdEx < l {
		preIndex := iNdEx
		...
		wireType := int(wire & 0x7)
		if wireType == 4 {
			return fmt.Errorf("proto: TriggerDescriptor: wiretype end group for non-group")
		}
		if fieldNum <= 0 {
			return fmt.Errorf("proto: TriggerDescriptor: illegal tag %d (wire type %d)", fieldNum, wire)
		}
		switch fieldNum {
		case 1:
			if wireType != 0 {
		...
		case 4:
			...
			m.Events = append(m.Events, &TriggerDescriptor_Event{})
		...
		case 8:
			...
			m.WhenExpr = Expression(dAtA[iNdEx:postIndex])
			iNdEx = postIndex
		case 9:
			if wireType != 0 {
		...
		case 11:
			...
			m.FuncBody = RoutineBody(dAtA[iNdEx:postIndex])
			iNdEx = postIndex
		case 12:
			if wireType != 0 {
			...
			if wireType == 0 {
				...
				if elementCount != 0 {
					if m.DependsOn == nil {
						...
						m.DependsOn = slices.Grow(m.DependsOn, elementCount)
					}
				}
				for iNdEx < postIndex {
					var v ID
		...
		case 14:
			if wireType == 0 {
				...
				elementCount = count
				if elementCount != 0 {
					if m.DependsOnTypes == nil {
						m.DependsOnTypes = make([]ID, 0, elementCount)
					} else {
						m.DependsOnTypes = slices.Grow(m.DependsOnTypes, elementCount)
					}
				}
				for iNdEx < postIndex {
					var v ID
		...
		case 15:
			if wireType == 0 {
				...
				m.DependsOnRoutines = append(m.DependsOnRoutines, v)
			} else if wireType == 2 {
				var packedLen int
				for shift := uint(0); ; shift += 7 {
					if shift >= 64 {
				...
				elementCount = count
				if elementCount != 0 {
					if m.DependsOnRoutines == nil {
						m.DependsOnRoutines = make([]ID, 0, elementCount)
					} else {
						m.DependsOnRoutines = slices.Grow(m.DependsOnRoutines, elementCount)
					}
				}
				for iNdEx < postIndex {
					var v ID
				...
				return fmt.Errorf("proto: wrong wireType = %d for field DependsOnRoutines", wireType)
			}
		default:
			iNdEx = preIndex
			skippy, err := skipStructured(dAtA[iNdEx:])
...
func (m *TriggerDescriptor_Event) Unmarshal(dAtA []byte) error {
	l := len(dAtA)
	iNdEx := 0
	for iNdEx < l {
		preIndex := iNdEx
			...
			if wireType == 0 {
				...
				if elementCount != 0 {
					if m.OldIndexes == nil {
						...
						m.OldIndexes = slices.Grow(m.OldIndexes, elementCount)
					}
				}
				for iNdEx < postIndex {
					var v IndexID
			...
			if wireType == 0 {
				...
				if elementCount != 0 {
					if m.NewIndexes == nil {
						...
						m.NewIndexes = slices.Grow(m.NewIndexes, elementCount)
					}
				}
				for iNdEx < postIndex {
					var v IndexID
...
func (m *ComputedColumnSwap) Unmarshal(dAtA []byte) error {
	...
	for iNdEx < l {
		...
		case 3:
			...
			m.InverseExpr = Expression(dAtA[iNdEx:postIndex])
			iNdEx = postIndex
		default:
			iNdEx = preIndex
			skippy, err := skipStructured(dAtA[iNdEx:])
...
func (m *PolicyDescriptor) Unmarshal(dAtA []byte) error {
	l := len(dAtA)
	iNdEx := 0
	for iNdEx < l {
		preIndex := iNdEx
		...
		wireType := int(wire & 0x7)
		if wireType == 4 {
			return fmt.Errorf("proto: PolicyDescriptor: wiretype end group for non-group")
		}
		if fieldNum <= 0 {
			return fmt.Errorf("proto: PolicyDescriptor: illegal tag %d (wire type %d)", fieldNum, wire)
		}
		switch fieldNum {
		case 1:
			if wireType != 0 {
		...
		case 3:
			...
			for shift := uint(0); ; shift += 7 {
				...
				b := dAtA[iNdEx]
				iNdEx++
				m.Type |= catpb.PolicyType(b&0x7F) << shift
				if b < 0x80 {
					break
				}
			}
		case 4:
			...
			for shift := uint(0); ; shift += 7 {
				...
				b := dAtA[iNdEx]
				iNdEx++
				m.Command |= catpb.PolicyCommand(b&0x7F) << shift
				if b < 0x80 {
					break
				}
			}
		...
		case 6:
			...
			m.UsingExpr = Expression(dAtA[iNdEx:postIndex])
			iNdEx = postIndex
		case 7:
			if wireType != 2 {
				return fmt.Errorf("proto: wrong wireType = %d for field WithCheckExpr", wireType)
			}
			var stringLen uint64
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
			...
			m.WithCheckExpr = Expression(dAtA[iNdEx:postIndex])
			iNdEx = postIndex
		case 8:
			if wireType == 0 {
				var v ID
		...
		case 9:
			if wireType == 0 {
				...
				elementCount = count
				if elementCount != 0 {
					if m.DependsOnFunctions == nil {
						m.DependsOnFunctions = make([]ID, 0, elementCount)
					} else {
						m.DependsOnFunctions = slices.Grow(m.DependsOnFunctions, elementCount)
					}
				}
				for iNdEx < postIndex {
					var v ID
		...
		case 10:
			if wireType == 0 {
				...
				m.DependsOnRelations = append(m.DependsOnRelations, v)
			} else if wireType == 2 {
				var packedLen int
				for shift := uint(0); ; shift += 7 {
					if shift >= 64 {
				...
				elementCount = count
				if elementCount != 0 {
					if m.DependsOnRelations == nil {
						m.DependsOnRelations = make([]ID, 0, elementCount)
					} else {
						m.DependsOnRelations = slices.Grow(m.DependsOnRelations, elementCount)
					}
				}
				for iNdEx < postIndex {
					var v ID
				...
				return fmt.Errorf("proto: wrong wireType = %d for field DependsOnRelations", wireType)
			}
		case 11:
			if wireType == 0 {
				var v ColumnID
				...
				m.UsingColumnIDs = append(m.UsingColumnIDs, v)
			} else if wireType == 2 {
				var packedLen int
				for shift := uint(0); ; shift += 7 {
					if shift >= 64 {
				...
				elementCount = count
				if elementCount != 0 {
					if m.UsingColumnIDs == nil {
						m.UsingColumnIDs = make([]ColumnID, 0, elementCount)
					} else {
						m.UsingColumnIDs = slices.Grow(m.UsingColumnIDs, elementCount)
					}
				}
				for iNdEx < postIndex {
					var v ColumnID
				...
				return fmt.Errorf("proto: wrong wireType = %d for field UsingColumnIDs", wireType)
			}
		case 12:
			if wireType == 0 {
				var v ColumnID
				...
				m.WithCheckColumnIDs = append(m.WithCheckColumnIDs, v)
			} else if wireType == 2 {
				var packedLen int
				for shift := uint(0); ; shift += 7 {
					if shift >= 64 {
				...
				elementCount = count
				if elementCount != 0 {
					if m.WithCheckColumnIDs == nil {
						m.WithCheckColumnIDs = make([]ColumnID, 0, elementCount)
					} else {
						m.WithCheckColumnIDs = slices.Grow(m.WithCheckColumnIDs, elementCount)
					}
				}
				for iNdEx < postIndex {
					var v ColumnID
				...
				return fmt.Errorf("proto: wrong wireType = %d for field WithCheckColumnIDs", wireType)
			}
		default:
			iNdEx = preIndex
			skippy, err := skipStructured(dAtA[iNdEx:])
...
func (m *DescriptorMutation) Unmarshal(dAtA []byte) error {
	...
	for iNdEx < l {
		...
		case 1:
			...
			alloc := new(struct {
				value DescriptorMutation_Column
		...
		case 2:
			...
			alloc := new(struct {
				value DescriptorMutation_Index
				field IndexDescriptor
			})
			if err := alloc.field.Unmarshal(dAtA[iNdEx:postIndex]); err != nil {
				return err
			}
			alloc.value.Index = &alloc.field
			m.Descriptor_ = &alloc.value
			iNdEx = postIndex
		case 3:
			if wireType != 0 {
		...
		case 8:
			...
			alloc := new(struct {
				value DescriptorMutation_Constraint
				field ConstraintToUpdate
			})
			if err := alloc.field.Unmarshal(dAtA[iNdEx:postIndex]); err != nil {
				return err
			}
			alloc.value.Constraint = &alloc.field
			m.Descriptor_ = &alloc.value
			iNdEx = postIndex
		case 9:
			if wireType != 2 {
			...
			alloc := new(struct {
				value DescriptorMutation_PrimaryKeySwap
				field PrimaryKeySwap
			})
			if err := alloc.field.Unmarshal(dAtA[iNdEx:postIndex]); err != nil {
				return err
			}
			alloc.value.PrimaryKeySwap = &alloc.field
			m.Descriptor_ = &alloc.value
			iNdEx = postIndex
		case 10:
			if wireType != 2 {
			...
			alloc := new(struct {
				value DescriptorMutation_ComputedColumnSwap
				field ComputedColumnSwap
			})
			if err := alloc.field.Unmarshal(dAtA[iNdEx:postIndex]); err != nil {
				return err
			}
			alloc.value.ComputedColumnSwap = &alloc.field
			m.Descriptor_ = &alloc.value
			iNdEx = postIndex
		case 11:
			if wireType != 2 {
			...
			alloc := new(struct {
				value DescriptorMutation_MaterializedViewRefresh
				field MaterializedViewRefresh
			})
			if err := alloc.field.Unmarshal(dAtA[iNdEx:postIndex]); err != nil {
				return err
			}
			alloc.value.MaterializedViewRefresh = &alloc.field
			m.Descriptor_ = &alloc.value
			iNdEx = postIndex
		case 12:
			if wireType != 2 {
			...
			alloc := new(struct {
				value DescriptorMutation_ModifyRowLevelTTL
				field ModifyRowLevelTTL
			})
			if err := alloc.field.Unmarshal(dAtA[iNdEx:postIndex]); err != nil {
				return err
			}
			alloc.value.ModifyRowLevelTTL = &alloc.field
			m.Descriptor_ = &alloc.value
			iNdEx = postIndex
		default:
			iNdEx = preIndex
...
func (m *TableDescriptor) Unmarshal(dAtA []byte) error {
	...
	for iNdEx < l {
		...
		case 24:
			...
			m.ViewQuery = Statement(dAtA[iNdEx:postIndex])
			iNdEx = postIndex
		case 25:
			if wireType == 0 {
				var v ID
		...
		case 34:
			...
			m.CreateQuery = Statement(dAtA[iNdEx:postIndex])
			iNdEx = postIndex
		case 35:
			if wireType != 2 {
		...
		case 62:
			if wireType == 0 {
				...
				m.LDRJobIDs = append(m.LDRJobIDs, v)
			} else if wireType == 2 {
				var packedLen int
				for shift := uint(0); ; shift += 7 {
					if shift >= 64 {
				...
				if elementCount != 0 {
					if m.LDRJobIDs == nil {
						m.LDRJobIDs = make([]github_com_cockroachdb_cockroach_pkg_sql_catalog_catpb.JobID, 0, elementCount)
					} else {
						m.LDRJobIDs = slices.Grow(m.LDRJobIDs, elementCount)
					}
				}
				for iNdEx < postIndex {
					var v github_com_cockroachdb_cockroach_pkg_sql_catalog_catpb.JobID
		...
		case 63:
			if wireType != 0 {
				return fmt.Errorf("proto: wrong wireType = %d for field ReplicatedPCRVersion", wireType)
			}
			m.ReplicatedPCRVersion = 0
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
					return ErrIntOverflowStructured
				}
				...
				b := dAtA[iNdEx]
				iNdEx++
				m.ReplicatedPCRVersion |= DescriptorVersion(b&0x7F) << shift
				if b < 0x80 {
					break
				}
			}
		case 64:
			...
			m.Triggers = append(m.Triggers, TriggerDescriptor{})
		...
		case 65:
			...
			for shift := uint(0); ; shift += 7 {
				...
				b := dAtA[iNdEx]
				iNdEx++
				m.NextTriggerID |= TriggerID(b&0x7F) << shift
				if b < 0x80 {
					break
				}
			}
		case 66:
			...
			m.Policies = append(m.Policies, PolicyDescriptor{})
		...
		case 67:
			...
			for shift := uint(0); ; shift += 7 {
				...
				b := dAtA[iNdEx]
				iNdEx++
				m.NextPolicyID |= PolicyID(b&0x7F) << shift
				if b < 0x80 {
					break
				}
			}
		case 68:
			...
			m.RowLevelSecurityEnabled = bool(v != 0)
		case 69:
			if wireType != 0 {
			...
			m.RowLevelSecurityForced = bool(v != 0)
		case 70:
			if wireType != 0 {
				return fmt.Errorf("proto: wrong wireType = %d for field RBRUsingConstraint", wireType)
			}
			m.RBRUsingConstraint = 0
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
					return ErrIntOverflowStructured
				}
				...
				b := dAtA[iNdEx]
				iNdEx++
				m.RBRUsingConstraint |= ConstraintID(b&0x7F) << shift
				if b < 0x80 {
					break
				}
			}
		case 71:
			if wireType != 0 {
				return fmt.Errorf("proto: wrong wireType = %d for field StatsCanaryWindow", wireType)
			}
			m.StatsCanaryWindow = 0
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
					return ErrIntOverflowStructured
				}
				...
				b := dAtA[iNdEx]
				iNdEx++
				m.StatsCanaryWindow |= time.Duration(b&0x7F) << shift
				if b < 0x80 {
					break
				}
			}
		case 72:
			if wireType != 0 {
				return fmt.Errorf("proto: wrong wireType = %d for field SecurityInvoker", wireType)
			}
			var v int
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
...
func (m *TableDescriptor_CheckConstraint) Unmarshal(dAtA []byte) error {
	...
	for iNdEx < l {
		...
		case 1:
			...
			m.Expr = Expression(dAtA[iNdEx:postIndex])
			iNdEx = postIndex
		case 2:
			if wireType != 2 {
				return fmt.Errorf("proto: wrong wireType = %d for field Name", wireType)
	...
	for iNdEx < l {
		...
		case 5:
			...
			for shift := uint(0); ; shift += 7 {
				...
				b := dAtA[iNdEx]
				iNdEx++
				m.TriggerID |= TriggerID(b&0x7F) << shift
				if b < 0x80 {
					break
				}
			}
...
func (m *TableDescriptor_SequenceOpts) Unmarshal(dAtA []byte) error {
	...
	for iNdEx < l {
		...
		case 7:
			if wireType != 0 {
				return fmt.Errorf("proto: wrong wireType = %d for field SessionCacheSize", wireType)
			}
			m.SessionCacheSize = 0
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
					return ErrIntOverflowStructured
				}
				...
				b := dAtA[iNdEx]
				iNdEx++
				m.SessionCacheSize |= int64(b&0x7F) << shift
				if b < 0x80 {
					break
				}
			}
...
func (m *ExternalRowData) Unmarshal(dAtA []byte) error {
	l := len(dAtA)
	iNdEx := 0
	for iNdEx < l {
		preIndex := iNdEx
		...
		wireType := int(wire & 0x7)
		if wireType == 4 {
			return fmt.Errorf("proto: ExternalRowData: wiretype end group for non-group")
		}
		if fieldNum <= 0 {
			return fmt.Errorf("proto: ExternalRowData: illegal tag %d (wire type %d)", fieldNum, wire)
		}
		switch fieldNum {
		case 1:
			if wireType != 2 {
...
func (m *SuperRegion) Unmarshal(dAtA []byte) error {
	...
	for iNdEx < l {
		...
		case 4:
			if wireType != 0 {
				return fmt.Errorf("proto: wrong wireType = %d for field HasSurvivalGoal", wireType)
			}
			var v int
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
...
func (m *ZoneConfigExtensions) Unmarshal(dAtA []byte) error {
	...
	for iNdEx < l {
		...
		case 4:
			if wireType != 2 {
				return fmt.Errorf("proto: wrong wireType = %d for field SuperRegion", wireType)
			}
			var msglen int
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
			...
			if m.SuperRegion == nil {
				m.SuperRegion = make(map[string]zonepb.ZoneConfig)
			}
			var mapkey string
			mapvalue := &zonepb.ZoneConfig{}
			for iNdEx < postIndex {
	...
	for iNdEx < l {
		...
		case 12:
			if wireType == 0 {
				...
				elementCount = count
				if elementCount != 0 {
					if m.ReferencingDescriptorIDs == nil {
						m.ReferencingDescriptorIDs = make([]ID, 0, elementCount)
					} else {
						m.ReferencingDescriptorIDs = slices.Grow(m.ReferencingDescriptorIDs, elementCount)
					}
				}
				for iNdEx < postIndex {
					var v ID
		...
		case 21:
			...
			if m.Domain == nil {
				m.Domain = &TypeDescriptor_Domain{}
			}
...
func (m *TypeDescriptor_Domain) Unmarshal(dAtA []byte) error {
	l := len(dAtA)
	iNdEx := 0
	for iNdEx < l {
		preIndex := iNdEx
		...
		case 2:
			if wireType != 0 {
				return fmt.Errorf("proto: wrong wireType = %d for field NotNullState", wireType)
			}
			m.NotNullState = 0
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
				...
				b := dAtA[iNdEx]
				iNdEx++
				m.NotNullState |= TypeDescriptor_Domain_NotNullState(b&0x7F) << shift
				if b < 0x80 {
					break
				}
			}
		...
		case 4:
			if wireType != 2 {
				return fmt.Errorf("proto: wrong wireType = %d for field CheckConstraints", wireType)
			}
			var msglen int
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
			...
			m.CheckConstraints = append(m.CheckConstraints, TypeDescriptor_Domain_CheckConstraint{})
			if err := m.CheckConstraints[len(m.CheckConstraints)-1].Unmarshal(dAtA[iNdEx:postIndex]); err != nil {
				return err
			}
			iNdEx = postIndex
		case 5:
			if wireType != 2 {
				return fmt.Errorf("proto: wrong wireType = %d for field NotNullConstraintName", wireType)
			}
			var stringLen uint64
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
			...
			m.NotNullConstraintName = string(dAtA[iNdEx:postIndex])
			iNdEx = postIndex
		case 6:
			if wireType != 0 {
				return fmt.Errorf("proto: wrong wireType = %d for field NotNullConstraintID", wireType)
			}
			m.NotNullConstraintID = 0
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
					return ErrIntOverflowStructured
				}
				...
				b := dAtA[iNdEx]
				iNdEx++
				m.NotNullConstraintID |= ConstraintID(b&0x7F) << shift
				if b < 0x80 {
					break
				}
			}
...
func (m *TypeDescriptor_Domain_CheckConstraint) Unmarshal(dAtA []byte) error {
	l := len(dAtA)
	iNdEx := 0
	for iNdEx < l {
		preIndex := iNdEx
	...
	for iNdEx < l {
		...
		case 6:
			if wireType == 0 {
				...
				elementCount = count
				if elementCount != 0 {
					if m.OutParamOrdinals == nil {
						m.OutParamOrdinals = make([]int32, 0, elementCount)
					} else {
						m.OutParamOrdinals = slices.Grow(m.OutParamOrdinals, elementCount)
					}
				}
				for iNdEx < postIndex {
					var v int32
...
func (m *FunctionDescriptor) Unmarshal(dAtA []byte) error {
	...
	for iNdEx < l {
		...
		case 8:
			...
			m.FunctionBody = RoutineBody(dAtA[iNdEx:postIndex])
			iNdEx = postIndex
		case 9:
			if wireType != 0 {
		...
		case 23:
			...
			for shift := uint(0); ; shift += 7 {
				...
				b := dAtA[iNdEx]
				iNdEx++
				m.Security |= catpb.Function_Security(b&0x7F) << shift
				if b < 0x80 {
					break
				}
			}
	...
	for iNdEx < l {
			...
			if wireType == 0 {
				...
				if elementCount != 0 {
					if m.IndexIDs == nil {
						...
						m.IndexIDs = slices.Grow(m.IndexIDs, elementCount)
					}
				}
				for iNdEx < postIndex {
					var v IndexID
		...
		case 4:
			if wireType == 0 {
				...
				elementCount = count
				if elementCount != 0 {
					if m.ConstraintIDs == nil {
						m.ConstraintIDs = make([]ConstraintID, 0, elementCount)
					} else {
						m.ConstraintIDs = slices.Grow(m.ConstraintIDs, elementCount)
					}
				}
				for iNdEx < postIndex {
					var v ConstraintID
		...
		case 5:
			if wireType == 0 {
				...
				for shift := uint(0); ; shift += 7 {
					...
					b := dAtA[iNdEx]
					iNdEx++
					v |= TriggerID(b&0x7F) << shift
					if b < 0x80 {
						break
					}
				}
				m.TriggerIDs = append(m.TriggerIDs, v)
			} else if wireType == 2 {
				var packedLen int
				for shift := uint(0); ; shift += 7 {
					if shift >= 64 {
				...
				if elementCount != 0 {
					if m.TriggerIDs == nil {
						m.TriggerIDs = make([]TriggerID, 0, elementCount)
					} else {
						m.TriggerIDs = slices.Grow(m.TriggerIDs, elementCount)
					}
				}
				for iNdEx < postIndex {
					var v TriggerID
				...
				return fmt.Errorf("proto: wrong wireType = %d for field TriggerIDs", wireType)
			}
		case 6:
			if wireType == 0 {
				var v PolicyID
				for shift := uint(0); ; shift += 7 {
					...
					b := dAtA[iNdEx]
					iNdEx++
					v |= PolicyID(b&0x7F) << shift
					if b < 0x80 {
						break
					}
				}
				m.PolicyIDs = append(m.PolicyIDs, v)
			} else if wireType == 2 {
				var packedLen int
				for shift := uint(0); ; shift += 7 {
					if shift >= 64 {
				...
				if elementCount != 0 {
					if m.PolicyIDs == nil {
						m.PolicyIDs = make([]PolicyID, 0, elementCount)
					} else {
						m.PolicyIDs = slices.Grow(m.PolicyIDs, elementCount)
					}
				}
				for iNdEx < postIndex {
					var v PolicyID
...
func (m *Descriptor) Unmarshal(dAtA []byte) error {
	...
	for iNdEx < l {
		...
		case 2:
			...
			alloc := new(struct {
				value Descriptor_Database
				field DatabaseDescriptor
			})
			if err := alloc.field.Unmarshal(dAtA[iNdEx:postIndex]); err != nil {
				return err
			}
		...
		case 5:
			...
			alloc := new(struct {
				value Descriptor_Function
				field FunctionDescriptor
			})
			if err := alloc.field.Unmarshal(dAtA[iNdEx:postIndex]); err != nil {
				return err
			}
...
func (m *DescriptorUpdates) Unmarshal(dAtA []byte) error {
	l := len(dAtA)
	iNdEx := 0
	for iNdEx < l {
		preIndex := iNdEx
		...
		wireType := int(wire & 0x7)
		if wireType == 4 {
			return fmt.Errorf("proto: DescriptorUpdates: wiretype end group for non-group")
		}
		if fieldNum <= 0 {
			return fmt.Errorf("proto: DescriptorUpdates: illegal tag %d (wire type %d)", fieldNum, wire)
		}
		switch fieldNum {
		case 1:
			if wireType == 0 {
				...
				if elementCount != 0 {
					if m.DescriptorIDs == nil {
						m.DescriptorIDs = make([]ID, 0, elementCount)
					} else {
		...
		case 2:
			if wireType == 0 {
				...
				for shift := uint(0); ; shift += 7 {
					...
					b := dAtA[iNdEx]
					iNdEx++
					v |= DescriptorVersion(b&0x7F) << shift
					if b < 0x80 {
						break
					}
				}
				m.DescriptorVersions = append(m.DescriptorVersions, v)
			} else if wireType == 2 {
				var packedLen int
				for shift := uint(0); ; shift += 7 {
					if shift >= 64 {
				...
				elementCount = count
				if elementCount != 0 {
					if m.DescriptorVersions == nil {
						m.DescriptorVersions = make([]DescriptorVersion, 0, elementCount)
					} else {
						m.DescriptorVersions = slices.Grow(m.DescriptorVersions, elementCount)
					}
				}
				for iNdEx < postIndex {
					var v DescriptorVersion
				...
				return fmt.Errorf("proto: wrong wireType = %d for field DescriptorVersions", wireType)
			}
		default:
			iNdEx = preIndex
			skippy, err := skipStructured(dAtA[iNdEx:])
